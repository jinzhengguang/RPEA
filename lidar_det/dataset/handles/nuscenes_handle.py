...
class NuScenesHandle:
    def __init__(self, data_dir, split, mini=False, nsweeps=1):
        ...
        scenes = np.array([all_scenes[all_scene_names.index(s)] for s in split_scenes])
        self._scenes = scenes
        ...
        self.__flat_sample_tokens = np.array(self.__flat_sample_tokens)
    ...
    def __getitem__(self, idx):
        ...
        anns = np.array([self._nusc.get("sample_annotation", a_t) for a_t in s_dict["anns"]])
